...
def normalize_artifacts_payload(artifacts):
    ...
    for key, default_value in DEFAULT_ARTIFACTS.items():
        value = artifacts.get(key, default_value)
        if isinstance(default_value, list):
            normalized[key] = value if isinstance(value, list) else deepcopy(default_value)
        elif isinstance(default_value, dict):
            merged_value = deepcopy(default_value)
...
def ask_openai_for_chat_response(messages, model=DEFAULT_MODEL):
    ...
    try:
        ...
        print(f"OpenAI request failed, using local fallback: {exc}")
        return None
...
def generate_structured_plan_with_ai(project_profile, inputs_json):
    ...
    if not isinstance(artifacts, dict) or not isinstance(narrative, str):
        return None
    normalized = normalize_artifacts_payload(artifacts)
...
def build_full_project_plan(project_profile, inputs_json, attachments=None):
    ...
    artifacts = normalize_artifacts_payload(artifacts)
